...
class ProjectMemberControllerTest {
	...
	void getProjectMembers_success() throws Exception {
		...
		when(projectMemberService.getProjectMembers(1L)).thenReturn(List.of(response));

		// when & then
		mockMvc.perform(get("/api/projects/{projectId}/members", 1L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.message").value("Project members retrieved successfully."))
				.andExpect(jsonPath("$.data[0].id").value(1)).andExpect(jsonPath("$.data[0].projectId").value(1))
	...
	void addProjectMember_success() throws Exception {
		...
		when(projectMemberService.addProjectMember(eq(1L), any(ProjectMemberAddRequest.class))).thenReturn(response);

		// when & then
		mockMvc.perform(post("/api/projects/{projectId}/members", 1L).contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(jsonPath("$.data.user.id").value(2)).andExpect(jsonPath("$.data.role").value("MEMBER"));
	...
	void removeProjectMember_success() throws Exception {
		...
		// when & then
		mockMvc.perform(delete("/api/projects/{projectId}/members/{userId}", 1L, 2L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
