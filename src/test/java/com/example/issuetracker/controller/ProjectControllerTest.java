...
public class ProjectControllerTest {
    ...
    void createProject_success() throws Exception {
        ...
        ProjectResponse response = createProjectResponse(
                1L,
                "Issue Tracker",
                "Issue tracker REST API project"
        );
        ...
        mockMvc.perform(post("/api/projects")
                ...
                .andExpect(jsonPath("$.data.name").value("Issue Tracker"))
    ...
    void updateProject_success() throws Exception {
        ...
        when(projectService.updateProject(eq(1L), any(ProjectUpdateRequest.class)))
                .thenReturn(response);

        // when & then
        mockMvc.perform(put("/api/projects/{projectId}", 1L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
    ...
    void getProjectStats_success() throws Exception {
        ...
        when(projectService.getProjectStats(1L))
                .thenReturn(response);

        // when & then
        mockMvc.perform(get("/api/projects/{projectId}/stats", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true))
                ...
                .andExpect(jsonPath("$.data.totalIssues").value(10))
