...
public class IssueControllerTest {
	...
	void createIssue_success() throws Exception {
		...
		when(issueService.createIssue(eq(1L), any(IssueCreateRequest.class))).thenReturn(response);

		// when & then
		mockMvc.perform(post("/api/projects/{projectId}/issues", 1L).contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(jsonPath("$.data.id").value(1)).andExpect(jsonPath("$.data.projectId").value(1))
				.andExpect(jsonPath("$.data.title").value("Login bug"))
				.andExpect(jsonPath("$.data.description").value("Login fails with 500 error"))
				.andExpect(jsonPath("$.data.status").value("TODO")).andExpect(jsonPath("$.data.priority").value("HIGH"))
	...
	void getIssuesByProject_success() throws Exception {
		// given
		IssueResponse issue1 = createIssueResponse(1L, 1L, "Login bug", "Login fails with 500 error", IssueStatus.TODO,
				IssuePriority.HIGH, LocalDate.of(2030, 12, 31), null);

		IssueResponse issue2 = createIssueResponse(2L, 1L, "UI task", "Update dashboard UI", IssueStatus.IN_PROGRESS,
				IssuePriority.MEDIUM, LocalDate.of(2030, 12, 31), null);

		when(issueService.getIssuesByProject(1L)).thenReturn(List.of(issue1, issue2));

		// when & then
		mockMvc.perform(get("/api/projects/{projectId}/issues", 1L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.message").value("Issues retrieved successfully."))
				.andExpect(jsonPath("$.data[0].id").value(1)).andExpect(jsonPath("$.data[0].title").value("Login bug"))
				.andExpect(jsonPath("$.data[0].status").value("TODO"))
				.andExpect(jsonPath("$.data[0].priority").value("HIGH")).andExpect(jsonPath("$.data[1].id").value(2))
	...
	void getIssue_success() throws Exception {
		...
		// when & then
		mockMvc.perform(get("/api/issues/{issueId}", 1L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
	...
	void updateIssue_success() throws Exception {
		...
		IssueResponse response = createIssueResponse(1L, 1L, "Updated issue", "Updated issue description",
				IssueStatus.IN_PROGRESS, IssuePriority.MEDIUM, LocalDate.of(2030, 12, 31), null);

		when(issueService.updateIssue(eq(1L), any(IssueUpdateRequest.class))).thenReturn(response);

		// when & then
		mockMvc.perform(put("/api/issues/{issueId}", 1L).contentType(MediaType.APPLICATION_JSON).content(requestBody))
				.andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.message").value("Issue updated successfully."))
				.andExpect(jsonPath("$.data.id").value(1)).andExpect(jsonPath("$.data.title").value("Updated issue"))
	...
	void deleteIssue_success() throws Exception {
		...
		// when & then
		mockMvc.perform(delete("/api/issues/{issueId}", 1L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
	...
	void searchIssuesByProject_success() throws Exception {
		// given
		IssueResponse issue = createIssueResponse(1L, 1L, "Login bug", "Login fails with 500 error", IssueStatus.TODO,
				IssuePriority.HIGH, LocalDate.of(2030, 12, 31), null);

		PageResponse<IssueResponse> response = new PageResponse<>(List.of(issue), 0, 10, 1L, 1, true, true);

		when(issueService.searchIssuesByProject(eq(1L), eq(IssueStatus.TODO), eq(IssuePriority.HIGH), eq("Login"),
				eq(0), eq(10), eq("id"), eq("desc"))).thenReturn(response);

		// when & then
		mockMvc.perform(get("/api/projects/{projectId}/issues/page", 1L).param("status", "TODO")
				.param("priority", "HIGH").param("keyword", "Login").param("page", "0").param("size", "10")
				...
				.andExpect(jsonPath("$.data.first").value(true)).andExpect(jsonPath("$.data.last").value(true));

		verify(issueService).searchIssuesByProject(eq(1L), eq(IssueStatus.TODO), eq(IssuePriority.HIGH), eq("Login"),
	...
	void updateIssueStatus_success() throws Exception {
		...
		IssueResponse response = createIssueResponse(1L, 1L, "Login bug", "Login fails with 500 error",
				IssueStatus.DONE, IssuePriority.HIGH, LocalDate.of(2030, 12, 31), null);

		when(issueService.updateIssueStatus(eq(1L), any(IssueStatusUpdateRequest.class))).thenReturn(response);

		// when & then
		mockMvc.perform(
				patch("/api/issues/{issueId}/status", 1L).contentType(MediaType.APPLICATION_JSON).content(requestBody))
				.andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true))
				.andExpect(jsonPath("$.message").value("Issue status updated successfully."))
				.andExpect(jsonPath("$.data.id").value(1)).andExpect(jsonPath("$.data.status").value("DONE"));
	...
	void assignIssue_success() throws Exception {
		...
		IssueResponse response = createIssueResponse(1L, 1L, "Login bug", "Login fails with 500 error",
				IssueStatus.TODO, IssuePriority.HIGH, LocalDate.of(2030, 12, 31),
				createUser(1L, "John Doe", "john@example.com", "john01"));

		when(issueService.assignIssue(eq(1L), any(IssueAssignRequest.class))).thenReturn(response);

		// when & then
		mockMvc.perform(patch("/api/issues/{issueId}/assignee", 1L).contentType(MediaType.APPLICATION_JSON)
				...
				.andExpect(jsonPath("$.data.id").value(1)).andExpect(jsonPath("$.data.assignee.id").value(1))
	...
	void unassignIssue_success() throws Exception {
		...
		when(issueService.unassignIssue(1L)).thenReturn(response);

		// when & then
		mockMvc.perform(delete("/api/issues/{issueId}/assignee", 1L)).andExpect(status().isOk())
				.andExpect(jsonPath("$.success").value(true))
	...
	private IssueResponse createIssueResponse(Long id, Long projectId, String title, String description,
			IssueStatus status, IssuePriority priority, LocalDate dueDate, User assignee) {
